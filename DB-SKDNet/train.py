...
# 保存检查点
def save_checkpoint(epoch, model, optimizer, best_iou, save_path):
    checkpoint = {
        'epoch': epoch,
...
def train1(train_loader, val_loader, Eva_train,  Eva_val, 
           data_name, save_path, net, criterion, 
           semicriterion, optimizer, use_KD, num_epoches):
    # # 初始化知识蒸馏损失
...
#----------------------------2.zszl半监督学习部分（损失计算分为两部分，一部分是有标签样本的损失，另一部分是无标签样本的半监督损失）------------------------------------------------
#----------------------------2.1.有标签样本损失计算------------------------------------------------
            else:#use_KD 为 True
                if with_label.any():
                    preds = net(A,B)
...
#--------------------------- 2.2.无标签样本损失计算-------------------------------------------------
    ...
    if new_iou >= best_iou:
        ...
        # torch.save(ema_net.state_dict(), save_path + '_train1_' + '_best_teacher_model.pth') #当student的精度最高的时候，同时存teacher的精度，然后用teacher的精度进行测试
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='SemiModel_noema04',
                        help='the test rgb images root')
    parser.add_argument('--save_path', type=str, default='./output/C2F-SemiCD/LEVIR5%-KD4/')  # 半监督的模型保存路径！！
    ...
    if opt.resume:
        ...
        if latest_checkpoint:
            ...
            start_epoch, best_iou = load_checkpoint(latest_checkpoint, model, optimizer)
    ...
    for epoch in range(start_epoch, opt.epoch):
        ...
        Eva_val.reset()
        # Eva_val2.reset()
        train1(train_loader, val_loader, Eva_train, Eva_val, data_name, save_path, model,
            criterion,semicriterion, optimizer,use_KD, opt.epoch)  # 需要理解
        # -----------------------------------------------------------------------------------------------------
        lr_scheduler.step()
        
        save_checkpoint(epoch, model, optimizer, best_iou, save_path)
