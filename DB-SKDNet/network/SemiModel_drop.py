...
class SemiModel_drop(nn.Module):
    def __init__(self, channel=32, dropout_p=0.5):
        super(SemiModel_drop, self).__init__()
#编码器
...
###############################################################################################     
        ...
        top = 0.5 + dropout_p / 2  # 强 Dropout 概率
        bottom = 0.5 - dropout_p / 2  # 弱 Dropout 概率
        print("-" * 25, f"Info: Using 3D Dropout with {top} (strong) and {bottom} (weak)", "-" * 25)
        self.strong_dropout = nn.Dropout3d(top).cuda()
        self.weak_dropout = nn.Dropout3d(bottom).cuda()
...
#解码器
    ...
    def forward(self, A, B,use_dropout=False,dropout_strength=None):
        layer1_A = self.inc(A)
...
###############################################################################################
        ...
        if use_dropout:
            ...
            if dropout_strength == "weak":
                layer1 = self.weak_dropout(layer1)
                layer2 = self.weak_dropout(layer2)
                layer3 = self.weak_dropout(layer3)
                layer4 = self.weak_dropout(layer4)
                layer5 = self.weak_dropout(layer5)
            elif dropout_strength == "strong":
                layer1 = self.strong_dropout(layer1)
                layer2 = self.strong_dropout(layer2)
                layer3 = self.strong_dropout(layer3)
                layer4 = self.strong_dropout(layer4)
                layer5 = self.strong_dropout(layer5)
            else:
                raise ValueError("dropout_strength must be 'weak' or 'strong'")
...
#     def __init__(self, channel=32, dropout_prob=0.5):
...
#         # --------------------------------
#         layer5_A = self.dropout(layer5_A)
#         # --------------------------------
#         layer1_B = self.inc(B)
#         layer2_B = self.down1(layer1_B)
#         layer3_B = self.down2(layer2_B)
...
#         # --------------------------------
#         layer5_B = self.dropout(layer5_B)
#         # --------------------------------
